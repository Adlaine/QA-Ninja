...
Então("deve ser adicionado {int} unidade\\(s) deste item") do |quantidade|
    ...
    expect(@rest_page.cart.box).to have_text "(#{quantidade}x) #{@produto_nome}" # interpolação de string para validar o produto no carrinh
end
...
Dado("que o produto desejado são:") do |table|                                
    @product_list = table.hashes #converteu a tabela para um array com todos os produtos
end                                                                           
...
Então("o valor total de itens de ser igual a {string}") do |total_itens|
